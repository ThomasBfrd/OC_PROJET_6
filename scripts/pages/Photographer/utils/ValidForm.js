...
setTimeout(() => {
	// Bouton de fermeture de la modal après confirmation d'envoi
	// du formulaire, appel de clearForm
	...
	// (inputs success / form reset / message de confirmation)
	// et fermeture de la modal
	const clearForm = () => {
	...
	if (submit) {
		submit.addEventListener('click', (event) => {
			// Ecoute du bouton submit, qui récupère les valeurs
			// de tous les champs appelés
			event.preventDefault();
			validForm();
		});
	}

	// Vérification du formulaire, si les données sont valides,
	// on appelle submitForm
	const validForm = () => {
		const firstNameValue = validString('firstName');
		const lastNameValue = validString('lastName');
		const emailValue = validEmail();
		const messageValue = validMessage();

		// si toutes les validations retournent true,
		// alors on peut appeler submitForm
	...
	// Données valides, on affiche alors le message de confirmation
	...
	// Vérifie si le prénom et nom match avec la regex,
	// et si les valeurs ne sont pas nulles
	...
	// Si la classe valid-input est présente,
	// alors on la supprime pour afficher le data-error
