...
export default class PhotographerProfile {
    ...
    createPhotographerCard(data) {
      ...
      const photographerById = photographerData.filter(photographer => photographer.id === +paramsId)

      let sectionPhotographer = document.querySelector('.photograph_header');
      ...
      <p class="city">${photographerById[0].city}, ${photographerById[0].country}</p>
      <p class="tagline">${photographerById[0].tagline}</p>
    </div>
    <button class="contact-button" role="button">Contactez-moi</button>
    <div class="imgUser">
    <img role="img" src="../../../assets/photographers/Photographers_ID_Photos/${photographerById[0].portrait}" alt="portrait photographe" aria-label="portrait photographer" class="portrait_${photographerById[0].id}">
    </div>
    `;
